...
class Corner_Predictor(nn.Module):
    ...
    def soft_argmax(self, score_map, return_dist=False, softmax=True):
        ...
        coord_x = self.coord_x.to(prob_vec.device)
        coord_y = self.coord_y.to(prob_vec.device)
...
class CenterPredictor(nn.Module, ):
    ...
    def cal_bbox_window(self, score_map_ctr_window, score_map_ctr, size_map, offset_map, return_score=False):
        max_score_window, idx = torch.max(score_map_ctr_window.flatten(1), dim=1, keepdim=True) # score_map_ctr.flatten(1): torch.Size([32, 256]) idx: torch.Size([32, 1]) max_score: torch.Size([32, 1])
        max_score = score_map_ctr.flatten(1)[0][idx[0][0]]
        idx_y = torch.div(idx, self.feat_sz, rounding_mode='floor')
        idx_x = idx % self.feat_sz
    ...
    def cal_bbox_sampling(self, idx, size_map, offset_map):
        idx = idx.unsqueeze(1)
        idx_y = torch.div(idx, self.feat_sz, rounding_mode='floor')
        idx_x = idx % self.feat_sz

        idx = idx.unsqueeze(1).expand(idx.shape[0], 2, 1)
        size = size_map.flatten(2).gather(dim=2, index=idx).squeeze(-1) # size_map: torch.Size([32, 2, 16, 16])  size_map.flatten(2): torch.Size([32, 2, 256])
        offset = offset_map.flatten(2).gather(dim=2, index=idx).squeeze(-1)
