...
# Adapted from the Fast-iTPN implementation.
...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        '''temporal tokens prompt'''
        self.ttoken_type = ttoken_type
        self.ttoken_num = ttoken_num
        if ttoken_block_idx == 'all':
            self.ttoken_block_idx = list(range(depth))
        else:
            self.ttoken_block_idx = ttoken_block_idx
        if self.ttoken_type in ['regist']:
            self.ttokens = nn.Parameter(torch.zeros(1, self.ttoken_num, embed_dim))
        ttoken_block_nums = len(self.ttoken_block_idx)
        self.ttoken_proj = ttoken_proj
        if self.ttoken_proj:
            self.ttoken_new_proj_layer = nn.Linear(embed_dim, embed_dim)
            self.ttoken_proj_layers = nn.ModuleList([
                nn.Linear(embed_dim, embed_dim)
                for _ in range(ttoken_block_nums)
            ])
        ...
        if use_abs_pos_emb:
            ...
            self.pos_embed_ttoken = nn.Parameter(torch.zeros(1, self.ttoken_num, embed_dim))
        else:
            self.pos_embed = None
            self.pos_embed_ttoken = None
        ...
        if self.pos_embed_ttoken is not None:
            trunc_normal_(self.pos_embed_ttoken, std=.02)
    ...
    def no_weight_decay(self):
        if self.cls_token is not None:
            return {'pos_embed', 'cls_token', 'pos_embed_ttoken'}
        return {'pos_embed', 'pos_embed_ttoken'}
    ...
    def prepare_tokens_with_masks(self, template_list, search_list, template_anno_list):
        ...
        '''concat new ttokens'''
        ttokens = self.ttokens + self.pos_embed_ttoken
        if self.ttoken_proj:
            ttokens = self.ttoken_new_proj_layer(ttokens)
        ttokens = ttokens.repeat(xz.size(0), 1, 1)
        xz = torch.cat([xz, ttokens, ttokens], dim=1)
        return xz

    def forward_features(self, template_list, search_list,template_anno_list, ttokens_prv):
        ...
        for i in range(self.num_main_blocks):
            if i in self.ttoken_block_idx:
                if ttokens_prv is not None:
                    xz = xz[:, :-1*self.ttoken_num, :].clone()
                    if self.ttoken_proj:
                        ttk_prv_i = self.ttoken_proj_layers[local_idx](ttokens_prv[local_idx])
                    else:
                        ttk_prv_i = ttokens_prv[local_idx]
                    xz = torch.cat([xz, ttk_prv_i], dim=1)
                ttokens_new = xz[:, -2*self.ttoken_num : -1*self.ttoken_num, :].clone()
                ttokens_update.append(ttokens_new)
                local_idx += 1
            blk = self.blocks[-self.num_main_blocks:][i]
        ...
        if self.fc_norm is not None:
            if self.cls_token is not None:
                xz = xz[:, 1:, :]
            return self.fc_norm(xz), ttokens_update
        else:
            return xz, ttokens_update

    def forward(self, template_list,search_list,template_anno_list, ttokens):
        xz, ttokens_update = self.forward_features(template_list,search_list,template_anno_list, ttokens)
        # x = self.head(x)
        out = [xz]
        return out, ttokens_update
