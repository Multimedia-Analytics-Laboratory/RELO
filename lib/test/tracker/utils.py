...
class Preprocessor(object):
    def __init__(self, device=None):
        ...
        self.mm_mean = torch.tensor(
            [0.485, 0.456, 0.406, 0.485, 0.456, 0.406],
            device=self.device,
        ).view((1, 6, 1, 1))
        self.mm_std = torch.tensor(
    ...
    def process(self, img_arr: np.ndarray, device=None):
        device = self.device if device is None else torch.device(device)
        if img_arr.shape[-1] == 6:
            mean = self.mm_mean.to(device)
            std = self.mm_std.to(device)
        else:
            mean = self.mean.to(device)
            std = self.std.to(device)
        # Deal with the image patch
        img_tensor = torch.as_tensor(img_arr, device=device).float().permute((2, 0, 1)).unsqueeze(dim=0)
