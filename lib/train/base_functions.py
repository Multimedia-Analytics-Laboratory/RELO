...
# datasets related
from lib.train.dataset import Lasot, Got10k, MSCOCOSeq, TrackingNet, VastTrack
...
def update_settings(settings, cfg):
    ...
    settings.train_type = train_type
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full",
                        "COCO17", "TRACKINGNET", "VASTTRACK"]
        if name == "LASOT":
            if settings.use_lmdb:
                raise NotImplementedError("LMDB dataset support is deferred.")
            else:
...
def build_dataloaders(cfg, settings):
    ...
    crop_mode = getattr(cfg.DATA, "CROP", None)
    if crop_mode == 'prev':
        data_processing_train = processing.ValidProcessing(search_area_factor=search_area_factor,
                                                          output_sz=output_sz,
                                                          ...
                                                          multi_modal_language=settings.multi_modal_language,
                                                          real_region = getattr(cfg.DATA, "REAL_REGION", False),
                                                          joint_aug = getattr(cfg.DATA, "JOINT_AUG", False),
                                                          settings=settings)
    else:
        data_processing_train = processing.SequenceProcessing(search_area_factor=search_area_factor,
                                                              output_sz=output_sz,
                                                              center_jitter_factor=settings.center_jitter_factor,
                                                              scale_jitter_factor=settings.scale_jitter_factor,
                                                              mode='sequence',
    ...
    if sampler_mode == 'consec_rl':
        raise NotImplementedError("Sequential RL loading is deferred.")
    ...
    if train_type == "peft":
        ...
        if is_main_process():
            ...
            for n, p in net.named_parameters():
                if p.requires_grad:
                    print(n)
    elif train_type == "rl_only":
        param_dicts = [
            {"params": [p for n, p in net.named_parameters() if (("policy_model" in n) or ("value_model" in n)) and p.requires_grad]},
        ]
        for n, p in net.named_parameters():
            if ("policy_model" not in n) and ("value_model" not in n):
                p.requires_grad = False
        if is_main_process():
            print("Learnable parameters are shown below.")
            for n, p in net.named_parameters():
