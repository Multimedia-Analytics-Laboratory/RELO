...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pos_prob=0.5, multi_modal_language=False, real_region=False):
        """
        args:
            ...
            train_cls - whether to sample classification pairs.
        ...
        while not valid:
            try:
                ...
                if is_video_dataset:
                    ...
                    elif self.frame_sample_mode == "consec":
                        template_frame_ids, search_frame_ids = self.get_frame_ids_consec(visible, seq_info_dict["valid"])
                    elif self.frame_sample_mode == "consec_rl":
                        template_frame_ids, search_frame_ids = self.get_frame_ids_consec_rl(visible, seq_info_dict["valid"])
                    ...
                    if self.real_region:
                        'if we use real region, we should cache the previous frame to get the bbox'
                        search_frame_ids.insert(0, search_frame_ids[0]-1)
                else:
                    # In case of image dataset, just repeat the image to generate synthetic video
                    template_frame_ids = [1] * self.num_template_frames
                    search_frame_ids = [1] * self.num_search_frames
                    'we do not need the previous frame for the image dataset'
                    # if self.real_region:
                    #     search_frame_ids.insert(0, 1)
                ...
                valid = data['valid']
                if any(img is None for img in data['search_images']):
                    valid = False
                if any(img is None for img in data['template_images']):
                    valid = False
    ...
    def get_frame_ids_consec_rl(self, visible, valid):

        template_frame_ids = []
        i = 0
        while (None in template_frame_ids) or (len(template_frame_ids) == 0):
            """we require the search frame to be valid but not necessary visible"""
            base_frame_id = self._sample_visible_ids(valid, num_ids=1,
                                                     min_id=self.num_template_frames,
                                                     max_id=len(valid) - self.num_search_frames + 1)
            search_frame_ids = list(range(base_frame_id[0], base_frame_id[0] + self.num_search_frames))

            """we require the template to be visible"""
            template_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_template_frames, min_id=0,
                                                          max_id=base_frame_id[0])
            template_frame_ids.sort(reverse=False)  # sort the template ids

            i += 1
            if i > 1000:
                print("cannot get frame ids, check the sampler")
            continue
    ...
    def get_frame_ids_consec(self, visible, valid):
        # get template and search ids in a 'consec' manner, the search ids are consecutive, while the template is not
        template_frame_ids = []
        i = 0
        while (None in template_frame_ids) or (len(template_frame_ids)==0):
            """we require the search frame to be valid but not necessary visible"""
            base_frame_id = self._sample_visible_ids(valid, num_ids=1,
                                                     min_id=self.num_template_frames, max_id=len(valid)-self.num_search_frames+1)
            search_frame_ids = list(range(base_frame_id[0], base_frame_id[0] + self.num_search_frames))

            """we require the template to be visible"""
            template_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_template_frames, min_id=0, max_id=base_frame_id[0])
            template_frame_ids.sort(reverse=False) #sort the template ids
